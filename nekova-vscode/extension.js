// =============================================================
// NEKOVA Language — VS Code Extension  (v1.10.0)
// =============================================================
// Commands:
...
function createStatusBar(context) {
    ...
    statusBar.tooltip = 'NEKOVA v1.10.0 — Connected Forge by SYNEKCOT Tech\nClick to run file';
...
function activate(context) {
    ...
    const shown = context.globalState.get('nekova.welcomeShown_1.10.0');
    if (!shown) {
        vscode.window.showInformationMessage(
            'NEKOVA v1.10.0 — docs site is live, plus think as <Shape>, cost/token tracking, converse: blocks for multi-turn AI dialogue, and a sandbox prompt-injection guard. Press F5 to run any .nk file.',
            'Open REPL', 'New Project'
        ).then(choice => {
            if (choice === 'Open REPL')    cmdOpenRepl();
            if (choice === 'New Project')  cmdNewProject();
        });
        context.globalState.update('nekova.welcomeShown_1.10.0', true);
    }
}
