// =============================================================
// NEKOVA Language — VS Code Extension  (v1.9.9)
// =============================================================
// Commands:
...
function createStatusBar(context) {
    ...
    statusBar.tooltip = 'NEKOVA v1.9.9 — Connected Forge by SYNEKCOT Tech\nClick to run file';
...
function activate(context) {
    console.log('NEKOVA Language extension v1.9.9 activated');
    ...
    const shown = context.globalState.get('nekova.welcomeShown_1.9.9');
    if (!shown) {
        vscode.window.showInformationMessage(
            'NEKOVA v1.9.9 — destructuring, keyword arguments, const, spread syntax, optional chaining (?.), enums, and a new Set type, plus more accurate error messages. Press F5 to run any .nk file.',
            'Open REPL', 'New Project'
        ).then(choice => {
            if (choice === 'Open REPL')    cmdOpenRepl();
            if (choice === 'New Project')  cmdNewProject();
        });
        context.globalState.update('nekova.welcomeShown_1.9.9', true);
    }
}
