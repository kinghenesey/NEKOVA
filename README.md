...
![Version](https://img.shields.io/badge/version-2.0.0-C41E0E?style=flat-square)
...
![Tests](https://img.shields.io/badge/tests-1712%20passing-success?style=flat-square)
...
### Self-Hosted Parser (Phase 27) — v2.0.0

The second self-hosting milestone: NEKOVA's parser is now written in
NEKOVA itself. `parser.nk` (~2,400 lines) is a complete recursive-descent
parser covering the full language — core statements and expressions,
the AI-native constructs, classes, `async`/`await`, `match`, and
`retry`/`observe`/`fallback` — verified against the Python reference
parser via structural AST diffing, including parsing its own source.

```bash
# Run any file through the self-hosted parser instead of the Python one
nekova run app.nk --self-hosted

# Or set it as the default for a project
# nekova.toml
[run]
self_hosted_parser = true
```

This release also cut the self-hosted parse path's own runtime roughly
in half (~16s → ~7.8s on `parser.nk` parsing itself) across four
profiling-driven passes, and fixed a real code-execution vulnerability
in the agent system's `calculate` tool (it used to call Python's
`eval()` directly on unsanitized input — see [CHANGELOG.md](CHANGELOG.md)
for both).
...
├── tests/                ← Test suite (1,712 tests across 27 phases)
├── main.py                ← Entry point
...
| 26b   | ✅     | Education layer — `nekova learn`, `nekova explain`, `nekova translate`, `nekova classroom`, `nekova help` glossary, `--simple-errors` |
| 26c   | ✅     | AI-native differentiators III — typed AI output validation + re-prompt, probabilistic testing, dollar-denominated think budgets, model fallback chains, deterministic AI-call replay (cassettes), capability-scoped agent sandboxing, `think_stream` |
| 27    | ✅     | **Self-hosting continues** — NEKOVA parser in NEKOVA (`parser.nk`), verified via AST diffing against the Python reference, including parsing its own source — v2.0.0 milestone |
