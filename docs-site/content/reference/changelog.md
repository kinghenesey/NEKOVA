...
Self-hosting milestone 2. NEKOVA's parser — not just its lexer — is now written in NEKOVA itself: `parser.nk` (~2,400 lines) covers every grammar layer, from core statements through the AI-native constructs to classes, `async`/`await`, `match`, and `retry`/`observe`/`fallback`. It's verified against the Python reference parser by diffing the two parsers' ASTs structurally on real-world input, including `parser.nk` parsing its own source. Run any file through it with `nekova run app.nk --self-hosted`, or set `self_hosted_parser = true` under `[run]` in `nekova.toml`. This release also cuts the self-hosted parse path's own runtime roughly in half (~16s → ~7.8s on that self-parsing benchmark) across four profiling-driven interpreter optimizations, and fixes a real code-execution vulnerability in the agent system's `calculate` tool, which used to call Python's `eval()` directly on unsanitized input — it's now a safe, whitelist-based arithmetic evaluator.
